...
4. **Nsjail seccomp**: applied via `--seccomp_string` to block `mount`, `pivot_root`, `unshare`, `bpf`, `kexec_load`, `keyctl`, `userfaultfd`, `io_uring_*`, and `ptrace` family inside the sandbox. Layered on top of the Docker-level seccomp profile.
